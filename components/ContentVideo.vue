<template>
    <div class="content">
        <div class="txt_box">
            ...
            <div class="explain">
                <span class="txt large">{{ videotxt01 }}</span>
                <span class="txt medium">{{ videotxt02 }}</span>
                <span class="txt medium mb1">{{ videotxt03 }}</span>
                <span class="txt light">{{ videotxt04 }}</span>
            </div>
        </div>
        <div class="work_box">
            <div class="slide_box" 
                ...
                <div class="work_container" 
                    ...
                    <div v-for="(video, index) in videos" :key="index" class="work">
...
const handleWheel = (event) => {
    ...
    // 휠 아래로 = 다음 슬라이드
    if (event.deltaY > 0) {
        if (currentPosition.value > -(slideWidth.value * (props.videos.length - 1))) {
...
const onDrag = (event) => {
    ...
    // 슬라이드 범위 제한
    const maxPosition = 0
    const minPosition = -(slideWidth.value * (props.videos.length - 1))
...
<style lang="scss" scoped src="~/assets/scss/content_video.scss"></style>